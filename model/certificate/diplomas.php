...
// Updated code using mb_convert_encoding()
$originalString = "Some UTF-8 encoded text";
$decodedString = mb_convert_encoding($originalString, 'ISO-8859-1', 'UTF-8');
  ...
  function Header()
  {
    ...
    if (isset($_GET['idUser']) & isset($_GET['idCurso']) & isset($_GET['tipA'])) {
      ...
      $consulta_reporte_alquiler = $conexion->query("SELECT PERSONA_NOMBRES,
            ...
            CARRERA_IDCURSO, DIPLOMA_TIPO_ASISTENTE, DIPLOMA_IMGDIPLOMA, CURSO_NOMBRE
            FROM TR_PERSONA tp
...
if (isset($_GET['idCurso'])) {
  ...
  $datos_reporte = $consulta_reporte_alquiler->fetch_object();
  if ($datos_reporte) {
    $pdf->Output('Certificado_' . $datos_reporte->CURSO_NOMBRE . '.pdf', 'I'); //nombreDescarga, Visor(I->visualizar - D->descargar)
  }
}